...
if x_13.size!=0:
    ...
    np.save("avg/13_mean",thirteen_avg)
...
if len(one[0])!=0:
    ...
    ax.imshow(one_avg.reshape((23,24)))
    ...
    plt.ylabel("dm(mag)")
    plt.title("Image: Averaged dmdt for class1 ("+str(x_1.shape[0])+") \nHeatmap: Averaged Change in Probabilities")
    plt.tight_layout()
    #plt.ticks.set_xspacing(0.0005*mul)
    plt.savefig("avg/1_mean_heatmap.png")
    plt.close()

if len(two[0])!=0:
    fig,ax=plt.subplots(1,1)
    ax.imshow(two_avg.reshape((23,24)))
    cb=ax.contourf(v,u,m_2_avg,15,cmap=inv_mycmap)
    plt.colorbar(cb)
    plt.xticks(xloc,dtints,rotation=90)
    plt.yticks(yloc,dmints)
    plt.xlabel("dt(days)")#,labelpad=20)
    plt.ylabel("dm(mag)")
    plt.title("Image: Averaged dmdt for class2 ("+str(x_2.shape[0])+") \nHeatmap: Averaged Change in Probabilities")
    plt.tight_layout()
    #plt.ticks.set_xspacing(0.0005*mul)
    plt.savefig("avg/2_mean_heatmap.png")
    plt.close()

if len(four[0])!=0:
    fig,ax=plt.subplots(1,1)
    ax.imshow(four_avg.reshape((23,24)))
    cb=ax.contourf(v,u,m_4_avg,15,cmap=inv_mycmap)
    plt.colorbar(cb)
    plt.xticks(xloc,dtints,rotation=90)
    plt.yticks(yloc,dmints)
    plt.xlabel("dt(days)")#,labelpad=20)
    plt.ylabel("dm(mag)")
    plt.title("Image: Averaged dmdt for class4 ("+str(x_4.shape[0])+") \nHeatmap: Averaged Change in Probabilities")
    plt.tight_layout()
    #plt.ticks.set_xspacing(0.0005*mul)
    plt.savefig("avg/4_mean_heatmap.png")
    plt.close()
    
if len(five[0])!=0:
    fig,ax=plt.subplots(1,1)
    ax.imshow(five_avg.reshape((23,24)))
    cb=ax.contourf(v,u,m_5_avg,15,cmap=inv_mycmap)
    plt.colorbar(cb)
    plt.xticks(xloc,dtints,rotation=90)
    plt.yticks(yloc,dmints)
    plt.xlabel("dt(days)")#,labelpad=20)
    plt.ylabel("dm(mag)")
    plt.title("Image: Averaged dmdt for class5 ("+str(x_5.shape[0])+") \nHeatmap: Averaged Change in Probabilities")
    plt.tight_layout()
    #plt.ticks.set_xspacing(0.0005*mul)
    plt.savefig("avg/5_mean_heatmap.png")
    plt.close()

if len(six[0])!=0:
    fig,ax=plt.subplots(1,1)
    ax.imshow(six_avg.reshape((23,24)))
    cb=ax.contourf(v,u,m_6_avg,15,cmap=inv_mycmap)
    plt.colorbar(cb)
    plt.xticks(xloc,dtints,rotation=90)
    plt.yticks(yloc,dmints)
    plt.xlabel("dt(days)")#,labelpad=20)
    plt.ylabel("dm(mag)")
    plt.title("Image: Averaged dmdt for class6 ("+str(x_6.shape[0])+") \nHeatmap: Averaged Change in Probabilities")
    plt.tight_layout()
    #plt.ticks.set_xspacing(0.0005*mul)
    plt.savefig("avg/6_mean_heatmap.png")
    plt.close()

if len(eight[0])!=0:
    fig,ax=plt.subplots(1,1)
    ax.imshow(eight_avg.reshape((23,24)))
    cb=ax.contourf(v,u,m_8_avg,15,cmap=inv_mycmap)
    plt.colorbar(cb)
    plt.xticks(xloc,dtints,rotation=90)
    plt.yticks(yloc,dmints)
    plt.xlabel("dt(days)")#,labelpad=20)
    plt.ylabel("dm(mag)")
    plt.title("Image: Averaged dmdt for class8 ("+str(x_8.shape[0])+") \nHeatmap: Averaged Change in Probabilities")
    plt.tight_layout()
    #plt.ticks.set_xspacing(0.0005*mul)
    plt.savefig("avg/8_mean_heatmap.png")
    plt.close()
    
if len(thirteen[0])!=0:
    fig,ax=plt.subplots(1,1)
    ax.imshow(thirteen_avg.reshape((23,24)))
    cb=ax.contourf(v,u,m_13_avg,15,cmap=inv_mycmap)
    plt.colorbar(cb)
    plt.xticks(xloc,dtints,rotation=90)
    plt.yticks(yloc,dmints)
    plt.xlabel("dt(days)")#,labelpad=20)
    plt.ylabel("dm(mag)")
    plt.title("Image: Averaged dmdt for class13 ("+str(x_13.shape[0])+") \nHeatmap: Averaged Change in Probabilities")
    plt.tight_layout()
    #plt.ticks.set_xspacing(0.0005*mul)
    plt.savefig("avg/13_mean_heatmap.png")
    plt.close()
