...
#sm=SMOTE(random_state=seed, ratio={2:1000,4:1000,5:1000,6:1000})
sm=SMOTE(random_state=seed, ratio={2:1000,4:1000,5:1000})
#sm=SMOTE(random_state=seed, ratio={3:1000,4:1000})
#tomeklinks=TomekLinks(random_state=seed, ratio={0:1000,1:1000,3:1000})

#(ff,gg,hh)=X_train.shape
#X_train,y_train=tomeklinks.fit_sample(X_train.reshape(ff,gg*hh),y_train)
...
layers = [
    ...
    (Conv2DLayer, {'name':'conv2d1', 'num_filters': 8, 'filter_size': (3, 3), 'pad': 0, 'nonlinearity':rectify}),
    (MaxPool2DLayer, {'name':'maxpool1','pool_size': (2, 2)}),
    #(DropoutLayer, {'name':'dropout1','p':0.1}),

    (Conv2DLayer, {'name':'conv2d2','num_filters': 16, 'filter_size': (5, 5), 'pad': 0, 'nonlinearity':rectify}),
    #(MaxPool2DLayer, {'pool_size': (2, 2)}),
    #(DropoutLayer, {'p':0.3}),         

    #(Conv2DLayer, {'name':'conv2d3','num_filters': 32, 'filter_size': (5, 5), 'pad': 0, 'nonlinearity':rectify}),
    #(MaxPool2DLayer, {'pool_size': (2, 2)}),
    #(DropoutLayer, {'p':0.5}),       
