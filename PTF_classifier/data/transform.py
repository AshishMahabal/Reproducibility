...
def compute_2d_representation(data):
    ...
    (smjd,smag) = list(zip(*sorted(list(zip(data['mjd'],data['mag'])))))
...
def _extract_images_features(args):
    ...
    try:
        ...
        # load data
        datafile = os.path.join(path, "indiv_lc", "class" + str(classnum), str(class_id) + '.txt')
        # print(datafile)
        # print(type(datafile))
        # print(data_aug)
        ...
        # extract features
            ...
            data = numpy.genfromtxt(datafile, dtype=[('lcid','S13'),('mjd','f8'),('mag','f8'),('magerr','f8')], comments="#",delimiter=" ")
            #print(helpers.dmdtim(data['mjd'],data['mag'],len(helpers.dmints),len(helpers.dtints)))
            #print(data['mjd'])
            #(smjd,smag) = zip(*sorted(zip(data['mjd'],data['mag'])))
            #print(smjd)
            #print(list(zip(*sorted(list(zip(data['mjd'],data['mag']))))))
            #(smjd,smag)=list(zip(*sorted(list(zip(data['mjd'],data['mag'])))))
            #print(smjd)
            #print(smag)
            #print(helpers.dmdtim(smjd,smag,len(helpers.dmints),len(helpers.dtints)))
        # extract 2d representation
            #(smjd,smag) = list(zip(*sorted(list(zip(data['mjd'],data['mag'])))))
            #dmdt2dar = helpers.dmdtim(smjd, smag, len(helpers.dmints), len(helpers.dtints))
            #images=dmdt2dar
        ...
        # extract features
            features = compute_features(data)
            #print(images)
            return(images,features,classnum)
...
def _get_all_lc(version=0, n_jobs=4,data_aug = "vanilla",classes=None,class_len=None):
    ...
    samples = [5000/i for i in class_len ]
    randoms = [5000%i for i in class_len ]
    ids = []
    counts = [0]*17
    ...
    return numpy.array(X_2d), numpy.array(X_features), numpy.array(y)    

def parse_dmdt_all(version=0,data_aug = "vanilla",classes=range(1,18),class_len=None):

    X_2d, X_features, y = _get_all_lc(version=version,data_aug = data_aug,classes=classes,class_len=class_len)

    d = os.path.join(_get_data_path(), "all")
    if not os.path.exists(d):
        os.makedirs(d)
    if data_aug=="sample_1":
        numpy.save(os.path.join(d, "X_2d_sample_1.npy"), X_2d)
        numpy.save(os.path.join(d, "X_features_sample_1.npy"), X_features)
        numpy.save(os.path.join(d, "y_sample_1.npy"), y)
...
if __name__ == "__main__":
    class_len = [10365, 1695, 0, 649, 1991, 168, 0, 522, 0, 0, 0, 0, 68, 0, 0, 0, 0]
